...
const P4_3_JOB_LOG_INDEX = ({ set_page_display }) => {
  ...
  // + PAGINATION PROCESS ========================================
  const [job_log_raw_data, set_job_log_raw_data] = useState([]);
  const [job_log_list, set_job_log_list] = useState([]);
  const [selected_list_id, set_selected_list_id] = useState("");
  const [refresh_job_log_list, set_refresh_job_log_list] = useState(false);
  ...
  const get_job_log_raw_data = async () => {
    try {
      set_is_loading(true);
      const response = await get(ref(db, `/DB1_BENBY_MERCH_APP/TBL_JOB_LOG`));
      const data = response.val();
      const data_array_job_log = Object.values(data);
      set_job_log_raw_data(data_array_job_log);
    } catch (error) {
      console.log(error);
    } finally {
  ...
  useEffect(() => {
    get_job_log_raw_data();
  }, []);

  useEffect(() => {
    filter_data(job_log_raw_data);
  }, [
    job_log_raw_data,
    current_page,
    search_query,
    refresh_job_log_list,
    sort_config,
  ]);

  const filter_data = (job_log_data) => {
    const filtered_data = apply_search_filter(job_log_data, search_query);
    ...
    set_job_log_list(indexed_data);
  };

  const apply_search_filter = (data, query) => {
    const fields_to_search = ["a1_ID", "b1_CATEGORY", "c1_DATE", "e1_STATUS"];
  ...
  const handle_sort = (key) => {
    ...
    set_refresh_job_log_list((prev) => !prev);
  };
  // - PAGINATION PROCESS ========================================
        ...
        <div className="w-100" style={{ height: "77vh" }}>
          ...
          >
            ...
            >
              ...
              >
                {render_thead("#", "", 10)}
                {render_thead("ID", "a1_ID", 15)}
                {render_thead("CATEGORY", "b1_CATEGORY", 20)}
                {render_thead("DATE / TIME", "c1_DATE", 20)}
                {render_thead("OSA HISTORY", "d1_OSA_HISTORY", 20)}
                {render_thead("MD HISTORY", "d2_MD_HISTORY", 20)}
                {render_thead("EP HISTORY", "d3_EP_HISTORY", 20)}
                {render_thead("STATUS", "e1_STATUS", 20)}
              </tr>
            </table>
          </div>
          <div
          ...
          >
            ...
            >
              <tbody>
                ...
                {!is_loading ? (
                  <React.Fragment>
                    {job_log_list.length !== 0 ? (
                      <React.Fragment>
                        {job_log_list.map((data) => {
                          return (
                            <tr
                              style={{
                                textAlign: "center",
                            ...
                            >
                              ...
                              {render_row(data.a1_ID, 15)}
                              {render_row(data.b1_CATEGORY, 20)}
                              {render_row(data.c1_DATE, 20)}
                              {render_row(data.d1_OSA_HISTORY, 20)}
                              {render_row(data.d2_MD_HISTORY, 20)}
                              {render_row(data.d3_EP_HISTORY, 20)}
                              {render_row(data.e1_STATUS, 20)}
                            </tr>
                          );
                        })}
                      </React.Fragment>
...
export default P4_3_JOB_LOG_INDEX;
