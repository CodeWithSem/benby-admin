...
const P2_4_MD_HISTORY_INDEX = ({ set_page_display }) => {
  ...
  const [refresh_md_history_list, set_refresh_md_history_list] =
    useState(false);
  ...
  const get_md_history_raw_data = async () => {
    try {
      set_is_loading(true);
          ...
          if (childPaths) {
            Object.keys(childPaths).forEach((childPath) => {
              ...
              if (info) {
                ...
                md_history_data.push(data);
              }
            });
          }
        });
      }
      set_md_history_raw_data(md_history_data);
    } catch (error) {
      console.log(error);
    } finally {
  ...
  useEffect(() => {
    ...
    current_page,
    search_query,
    refresh_md_history_list,
    sort_config,
  ]);

  const filter_data = (md_history_data) => {
    const filtered_data = apply_search_filter(md_history_data, search_query);
    ...
    set_md_history_list(indexed_data);
  };
  ...
  const handle_sort = (key) => {
    ...
    set_refresh_md_history_list((prev) => !prev);
  };
  // - PAGINATION PROCESS ========================================
        ...
        <div className="w-100" style={{ height: "77vh" }}>
          ...
          >
            ...
            >
              ...
              >
                {render_thead("#", "", 10)}
                {render_thead("BENBY ID", "benbyId", 22)}
                {render_thead("TDS ID", "tdsID", 22)}
                {render_thead("DISER ID", "diserID", 22)}
                {render_thead("DISER NAME", "diserName", 50)}
                {render_thead("DATE", "datetoday", 20)}
                {render_thead("REMARKS 1", "remarks1", 20)}
                {render_thead("REMARKS 2", "remarks2", 20)}
                {render_thead("REMARKS 3", "remarks3", 20)}
              </tr>
            </table>
          </div>
          <div
          ...
          >
            ...
            >
              <tbody>
                ...
                {!is_loading ? (
                  <React.Fragment>
                    {md_history_list.length !== 0 ? (
                      <React.Fragment>
                        {md_history_list.map((data) => {
                          return (
                            <tr
                              style={{
                                textAlign: "center",
                            ...
                            >
                              ...
                              {render_row(data.benbyId, 22)}
                              {render_row(data.tdsID, 22)}
                              {render_row(data.diserID, 22)}
                              {render_row(data.diserName, 50)}
                              {render_row(data.datetoday, 20)}
                              {render_row(data.remarks1, 20)}
                              {render_row(data.remarks2, 20)}
                              {render_row(data.remarks3, 20)}
                            </tr>
                          );
                        })}
                      </React.Fragment>
...
export default P2_4_MD_HISTORY_INDEX;
