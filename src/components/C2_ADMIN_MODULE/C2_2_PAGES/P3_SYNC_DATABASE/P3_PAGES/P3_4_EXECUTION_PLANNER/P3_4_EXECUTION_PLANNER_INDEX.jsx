...
const P3_4_EXECUTION_PLANNER_INDEX = ({ set_page_display }) => {
  ...
  const [abort_controller_get_EP, set_abort_controller_get_EP] = useState(null);

  const [db_ep_list, set_db_ep_list] = useState([]);
  const [is_get_EP_loading, set_is_get_EP_loading] = useState(false);
  const [show_get_EP_alert, set_show_get_EP_alert] = useState(false);
  const [show_push_EP_alert, set_show_push_EP_alert] = useState(false);
  const [data_exist, set_data_exist] = useState(false);

  const get_EP = async () => {
    set_db_ep_list([]);
    const controller = new AbortController();
    set_abort_controller_get_EP(controller); // Store the controller for later use

    set_is_get_EP_loading(true);
    set_show_get_EP_alert(true);
    ...
    try {
      ...
      set_show_get_EP_alert(false);
    }
  };

  // Cancel function
  const cancel_get_EP = () => {
    if (abort_controller_get_EP) {
      abort_controller_get_EP.abort(); // Cancel the request
      set_abort_controller_get_EP(null); // Reset the controller after aborting
    }
  };
  // - GET METHOD MERCH DEPLOYMENT
  ...
  useEffect(() => {
    filter_data(db_ep_list);
  }, [db_ep_list, current_page, search_query, refresh_ep_list, sort_config]);
...
export default P3_4_EXECUTION_PLANNER_INDEX;
