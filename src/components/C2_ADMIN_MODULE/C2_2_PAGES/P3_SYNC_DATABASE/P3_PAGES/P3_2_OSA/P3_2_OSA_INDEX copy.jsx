...
const P3_2_OSA_INDEX = ({ set_page_display }) => {
  ...
  // + GET METHOD OSA
  const [abort_controller_get_OSA, set_abort_controller_get_OSA] =
    useState(null);

  const [db_osa_list, set_db_osa_list] = useState([]);
  const [is_get_OSA_loading, set_is_get_OSA_loading] = useState(false);
  const [show_get_OSA_alert, set_show_get_OSA_alert] = useState(false);
  const [show_push_OSA_alert, set_show_push_OSA_alert] = useState(false);
  const [data_exist, set_data_exist] = useState(false);

  const get_OSA = async () => {
    set_db_osa_list([]);
    const controller = new AbortController();
    set_abort_controller_get_OSA(controller); // Store the controller for later use

    set_is_get_OSA_loading(true);
    set_show_get_OSA_alert(true);
    ...
    try {
      ...
      set_is_get_OSA_loading(false);
      set_show_get_OSA_alert(false);
    }
  };

  // Cancel function
  const cancel_get_OSA = () => {
    if (abort_controller_get_OSA) {
      abort_controller_get_OSA.abort(); // Cancel the request
      set_abort_controller_get_OSA(null); // Reset the controller after aborting
    }
  };
  // - GET METHOD OSA
  ...
  useEffect(() => {
    filter_data(db_osa_list);
  }, [db_osa_list, current_page, search_query, refresh_OSA_list, sort_config]);
  ...
  const handle_push_to_cloud_osa = (data) => {
    const controller = new AbortController();
    set_abort_controller(controller);

    push_to_cloud_osa(data, 1000, 500, controller);
  };

  const cancel_push_to_cloud_osa = () => {
    if (abort_controller) {
      ...
      <React.Fragment>
        ...
        >
          ...
          <div style={{ color: "var(--text-color)", fontSize: "1.8vh" }}>
            Fetching OSA Data from database...
          </div>
          <div
            style={{
              width: "100%",
        ...
        >
          ...
          >
            <button
              ...
              onClick={cancel_push_to_cloud_osa}
            >
              CANCEL
            </button>
          </div>
  ...
  return (
    <React.Fragment>
        ...
        <div className="admin-page-label">
          ...
          >
            ...
            <button
              ...
              style={{ padding: "0 2vh" }}
              onClick={() => {
                handle_push_to_cloud_osa(db_osa_list);
              }}
              disabled={
                is_get_OSA_loading || db_osa_list.length === 0 ? true : false
              }
            >
              PUSH to Cloud
            </button>
      ...
      {is_get_OSA_loading ? RENDER_LOADING_MODAL() : null}
      {show_push_OSA_alert ? RENDER_PUSH_MODAL() : null}
    </React.Fragment>
  );
};

export default P3_2_OSA_INDEX;
